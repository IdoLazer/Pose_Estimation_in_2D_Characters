...
def finite_diff_grad(img):
    ...
    kernel_y = torch.tensor([[[[0, 1, 0],
                               [0, 0, 0],
                               [0, -1, 0]]]], dtype=img.dtype).to(img.device)
    div_x = nn.functional.conv2d(img, kernel_x, padding=1)
    div_y = nn.functional.conv2d(img, kernel_y, padding=1)
...
def finite_diff_div(grad):
    ...
    div_x = nn.functional.conv2d(grad[:, 0:1, ...], kernel_x, padding=1)
    div_y = nn.functional.conv2d(grad[:, 1:, ...], kernel_y, padding=1)
...
def heat_method(image, timestep=1.0, mass=.01, iters_diffusion=500, iters_poisson=1000):
    ...
    grad = grad / (grad.norm(dim=1).unsqueeze(1)).clamp(1e-12)  # normalize gradient
